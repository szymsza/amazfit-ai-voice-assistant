...
- **Server `/api/ask` uses `application/json`:** flat JSON body — `{ audio: string (base64 OPUS), groqKey, llmProvider?, llmModel?, llmKey?, ttsVoice?, maxTurns?, conversation? }`. Zepp OS `fetch()` does not support `FormData` multipart — it drops the body and sends `application/json` with no content. Side Service sends JSON directly with explicit `Content-Type: application/json` header.
